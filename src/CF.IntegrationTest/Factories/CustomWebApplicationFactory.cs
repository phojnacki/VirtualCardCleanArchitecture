...
public class CustomWebApplicationFactory : WebApplicationFactory<Program>
{
    ...
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        builder.ConfigureServices(services =>
        {
            ...
            {
                typeof(DbContextOptions<VirtualCardContext>)
            };
            ...
            services.AddDbContext<VirtualCardContext>(options =>
            {
            ...
            using var dbContext = scope.ServiceProvider.GetRequiredService<VirtualCardContext>();
            dbContext.Database.EnsureCreated();
            var logger = scope.ServiceProvider.GetRequiredService<ILogger<CustomWebApplicationFactory>>();

            try
            {
                Task.FromResult(VirtualCardSeed.Populate(dbContext));
            }
            catch (Exception ex)
            {
